// Frontend script for NBA career 3PT% game mode
...
async function fetchHighScore() {
  const response = await fetch(
    "http://localhost:5000/players/highscore/3ptpercent",
    {
...
async function fetchPlayers() {
  ...
  document.getElementById("player1").innerHTML = `
    ...
    <img draggable="false" src="${player1.image_url}" alt="${player1.player_name}" class="player-image" onclick="checkWinner(${player1["3ptpercent"]}, ${player2["3ptpercent"]}, 1)">
  `;

  document.getElementById("player2").innerHTML = `
    <h2>${player2.player_name}</h2>
    <img draggable="false" src="${player2.image_url}" alt="${player2.player_name}" class="player-image" onclick="checkWinner(${player1["3ptpercent"]}, ${player2["3ptpercent"]}, 2)">
  `;
